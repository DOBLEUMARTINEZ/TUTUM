	...
	for ($i=0; $i < 5; $i++) { 
		 ...
		 if ($i==0) {
			...
			<div id="box-img-gallery-<?=$i?>" >
				...
				<img id="document-<?=$i;?>" alt="your image" src="./images/icon/icon-texture.png" />

				<input type="file" name="docs-<?=$i;?>" id="docs-<?=$i;?>" class="inputfile inputfile-1" data-multiple-caption="{count} archivos seleccionados" multiple onchange="document.getElementById('document-<?=$i;?>').src = window.URL.createObjectURL(this.files[0])">
