<!-- BACKGROUND SECTION -->
<div class="body-content">

    <span class="background-section-darkness"></span>
    <span class="lateral-background latera-menu-off"></span>

	<!-- CONTENIDO LATERAL -->
	<div class="lateral-content">


		<!-- CONTENIDO PROYECTOS INTEGRALES TI -->
		<a id="proyectos"></a>
		<div>

			<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>PROYECTOS<br> INTEGRALES​</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>

			<img class="icon-float" src="images/icons/dispositivo.png">
			<span style="background: url('images/img/FOTO 07.jpg');"></span>
		  	<h2>DESARROLLO E IMPLEMENTACIÓN DE SOLUCIONES EN TI</h2>
		  	<p>
		  		Ofrecemos nuestra experiencia y confiablidad generada a través de los años para ofrecerte soluciones completas tanto de hardware como de software que incluyen el diseño, suministro, desarrollo e implementación de soluciones que mejor se adapte a tus necesidades, garantizando en todo momento la puesta en funcionamiento de la misma.
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3> <i class="fas fa-network-wired"></i> Servicios:</h3>
				  		<p style="text-align: left;">
				  			* Soluciones integrales en TI​<br>
				  			* Desarrollo de ingenierías​<br>
				  			* Suministro de equipamiento​<br>
				  			* Implementación de sistemas​<br>
				  			* Capacitación usuario final​<br>
				  			* Implementación SAP​<br>
				  			* Implementación HIS​<br>
				  			* Pólizas<br>
				  		</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/dispositivo.png">
				  		<p>Desarrollamos soluciones integrales de TI con la mas alta calidad</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/checklist.png">
				  		<p>Vamos de la mano con nuestros clientes para conocer y atender las necesidades de su proyecto</p>
				  	</li>

				</ol>

		  	</div>
		</div>
		<div>
			<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>PROYECTOS<br> INTEGRALES​</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
			<img class="icon-float" src="images/icons/dispositivo.png">
			<span style="background: url('images/img/FOTO 08.jpg');"></span>
		  	<h2>CATÁLOGO DE SOLUCIONES</h2>
		  	<p>
		  		Contamos con experiencia en implementación de más de 25 soluciones en TI, con la finalidad de ofrecer al mercado la tecnología más avanzada y la mejor oferta en sistemas, software y hardware.
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<p style="text-align: left;">
				  			* Cableado Estructurado​<br>
				  			* Telefonía E Informática​<br>
				  			* Computo E Impresión​<br>
				  			* Sistema De Redes Y Wifi​<br>
				  			* Circuito Cerrado De Televisión​<br>
				  			* Detección De Incendios​<br>
				  			* Control De Acceso​<br>
				  			* Voceo Y Sonorización​<br>
				  			* Intercomunicación<br>
				  		</p>
				  	</li>

				  	<li>
				  		<p style="text-align: left;">
				  			* Voceo IP​<br>
				  			* Detección De Sustancias Y Objetos​<br>
				  			* Sensores Perimetrales​<br>
				  			* Videoconferencias​<br>
				  			* Inhibición Celular​<br>
				  			* Control, Y Monitoreo (BMS)​<br>
				  			* Integración De Sistemas ​<br>
				  			* Radiocomunicación Segura<br>
				  		</p>
				  	</li>

				  	<li>
				  		<p style="text-align: left;" >
				  			* Tele Enseñanza<br>
				  			* Tele Medicina<br>
				  			* Televisión IP<br>
				  			* Control Neonatal RFID<br>
				  			* Sistemas De Información Hospitalaria (HIS)<br>
				  			* Planificación De Recursos Empresariales (ERP)<br>
				  			* Ciberseguridad<br>
				  		</p>
				  	</li>

				</ol>

		  	</div>
		</div>

	  	<!-- CONTENIDO OPERACIÓN DE PROYECTOS​ -->
	  	<a id="operacion"></a>
	  	<div>
	  		<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>OPERACIÓN DE<br> PROYECTOS</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
	  		<img class="icon-float" src="images/icons/dashboard.png">
			<span style="background: url('images/img/FOTO 09.jpg');"></span>
		  	<h2>OPERACIÓN DE PROYECTOS </h2>
		  	<p>
		  		TUTUM TECH, asegura la continuidad de la operación de la infraestructura de TI de sus clientes. A través de los diversos servicios especializados de TUTUM en servicios administrados y operación, logramos que la organización se enfoque en el desarrollo del negocio, mejorando la eficiencia y la confiabilidad de las operaciones de tecnología.
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3> <i class="fas fa-network-wired"></i> Servicios:</h3>
				  		<p style="text-align: left;">
				  			* Infraestructura tecnológica<br>
				  			* Administración<br>
				  			* Operación <br>
				  			* Refaccionamiento<br>
				  			* Capacitación<br>
				  			* Ciberseguridad<br>
				  			* PAET<br>
				  			* CAU<br>
				  			* HIS <br>
				  			* ERP<br>
				  		</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/team.png">
				  		<h3 style="justify-content: center;">+200</h3>	
				  		<p style="text-align: center;" >Especialistas operan nuestros sistemas todos los días</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/headphones.png">
				  		<p style="text-align: center;">Servicios 24/7​<br>Soportados por</p>
				  		<h3 style="justify-content: center; margin: 0; font-size: 2.5vw">nuestro</h3>
				  		<p style="text-align: center;">Centro de atención al usuario​</p>
				  	</li>

				</ol>

		  	</div>
		</div>

	  	<!-- CONTENIDO CENTRO DE ATENCIÓN A USUARIOS​ -->
	  	<a id="cau"></a>
	  	<div>
	  		<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>CENTRO DE ATENCIÓN A USUARIOS</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
	  		<img class="icon-float" src="images/icons/headphones.png">
			<span style="background: url('images/img/FOTO 10.jpg');"></span>
		  	<h2>CENTRO DE ATENCIÓN A USUARIOS​</h2>
		  	<p>
		  		Nuestra herramienta <strong>“Remedy Service Desk”</strong>, nos permite implementar una mesa de ayuda a nivel Enterprise, la cual es compatible con los procesos de ITIL para el manejo de incidentes, manejo de problemas, brindar niveles de servicios y ordenes de trabajo.
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3><i class="fas fa-network-wired"></i> Servicios:</h3>
				  		<p style="text-align: left;">
				  			* Administración<br>
				  			* Soporte a la operación en los 3 niveles<br>
				  			* Refaccionamiento<br>
				  			* Capacitación<br>
				  			* Soluciones empresariales<br>
				  			* Diseño distribuido en la nube<br>
				  			* Data center<br>
				  			* Pólizas<br>
				  		</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/headphones.png">
				  		<p style="text-align: center;">Técnico<br> ​Data Center<br>​ Seguridad Perimetral</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/checklist.png">
				  		<p style="text-align: center;">PROCESOS</p>
				  		<h3 style="justify-content: center;font-size: 2.5vw; margin: 0;">ITIL</h3>
				  	</li>

				</ol>

		  	</div>
		</div>
		<div>
			<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>CENTRO DE ATENCIÓN A USUARIOS</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
			<span style="background: url('images/img/FOTO 12.jpg');"></span>
		  	<h2>CENTRO DE ATENCIÓN A USUARIOS​</h2>
		  	<br>

		  	<div>
		  		
				<ol class="square-cau-02">	
				  	<li>
				  		<h3>Gestión en asistencia a usuarios en los 3 niveles de soporte:<br><br>Actividades :</h3>
				  		<p style="text-align: left;">
				  			* Administración del Directorio Activo y licenciamiento.<br>
				  			* Inventarios de equipo de computo.<br>
				  			* Mantenimientos preventivos.<br>
				  			* Actualización de sistemas.<br>
				  			* Respaldos de información.<br>
				  			* Diagnóstico en el mal funcionamiento del hardware y el software.<br>
				  			* Seguimiento de tickets mediante la Mesa de Ayuda.<br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Gestión de proyectos e implementaciones  a los sistemas de seguridad perimetral:<br><br>Actividades :</h3>
				  		<p style="text-align: left;">
				  			* Soporte a las incidencias, problemáticas de los clientes<br>
				  			* Ofrecemos Capacitación Técnica<br>
				  			* Elaboración de reportes y manuales​<br>
				  			* Mantenimientos preventivos y correctivos<br>
				  			* Administración de CCTV Corporativo ​<br>
				  			* Implementación de soluciones de Seguridad <br>
				  			&nbsp;&nbsp;&nbsp;&nbsp;* Hospitales<br>
				  			&nbsp;&nbsp;&nbsp;&nbsp;* Escuela Naval<br>
				  			* Detección de incendios, Controles de Acceso<br>
				  			* Voceo, BMS <br>
				  			* Seguimiento de tickets mediante la Mesa de Ayuda<br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Gestión de proyectos e implementaciones  a los sistemas de seguridad perimetral:<br><br>Actividades :</h3>
				  		<p style="text-align: left;">
				  			* Soporte a las incidencias, problemáticas de los clientes<br>
				  			* Ofrecemos Capacitación Técnica<br>
				  			* Elaboración de reportes y manuales​<br>
				  			* Mantenimientos preventivos y correctivos<br>
				  			* Administración de CCTV Corporativo ​<br>
				  			* Implementación de soluciones de Seguridad <br>
				  			&nbsp;&nbsp;&nbsp;&nbsp;* Hospitales<br>
				  			&nbsp;&nbsp;&nbsp;&nbsp;* Escuela Naval<br>
				  			* Detección de incendios, Controles de Acceso<br>
				  			* Voceo, BMS <br>
				  			* Seguimiento de tickets mediante la Mesa de Ayuda<br>
				  		</p>
				  	</li>

				</ol>

		  	</div>
		</div>

	  	<!-- CONTENIDO CONSULTORÍA E IMPLEMENTACIÓN SA​P -->
	  	<a id="sap"></a>
	  	<div>
	  		<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>CONSULTORÍA E IMPLEMENTACIÓN SAP</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
	  		<img class="icon-float" src="images/icons/document.png">
			<span style="background: url('images/img/FOTO 11.jpg');"></span>
		  	<h2>CONSULTORÍA E IMPLEMENTACIÓN SAP</h2>
		  	<p>
		  		Ofrecemos soluciones para la planificación de recursos empresariales, que ayuda a ejecutar procesos centrales en un único sistema para departamentos como finanzas, fabricación, RR. HH., cadena de suministro, servicios, compras y otros. Nuestras soluciones de ERP utilizan tecnologías inteligentes para ayudar a nuestros clientes a transformar los procesos de misión crítica y adaptarse rápidamente al cambio sin importar el tamaño de su negocio.
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3>METODOLOGÍAS DE IMPLEMENTACIÓN</h3>
				  		<p style="text-align: left;" >
				  			* PMI<br>
				  			* COSO<br>
				  			* KANBAN<br>
				  			* SPRINT<br>
				  			* KOTTER<br>
				  			* SAP ACTIVATE<br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>MÓDULOS SAP</h3>
				  		<p style="text-align: left;" >
				  			* <strong>EP</strong> Enterprise Portals<br>
				  			* <strong>BI</strong> Bussines Intelligence<br>
				  			* <strong>MDM</strong> Master Data Manager<br>
				  			* <strong>IDM</strong> Identity Management<br>
				  			* <strong>MI</strong> Mobile Infrastructure<br>
				  			* <strong>ACC</strong> Adaptative Computing Controlling<br>
				  			* <strong>ES</strong> Enterprice Services<br>
				  			* <strong>PI</strong> Process Integration<br>
				  			* <strong>BO</strong>  Business Objects<br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>APLICACIONES</h3>
				  		<p style="text-align: left;" >
				  			* <strong>MMI</strong> Manufacturing Integration And Intelligence<br>
				  			* <strong>SRM</strong> Supplier Relationship Management<br>
				  			* <strong>SCM</strong> Supply Chain Management<br>
				  			* <strong>SOLMAN</strong> Solmgr Solution Manager<br>
				  			* <strong>PLM</strong>  Product Lifecycle Managment<br>
				  		</p>
				  	</li>

				  	<li style="display: none;">
				  		<h3>CONSULTORÍA EN SOLUCIONES SAP</h3>
				  		<p style="text-align: left;" >
				  			* Soluciones en la cadena de suministro, logística ​ <br>
							* Soluciones Financieras y de costos ​ <br>
							* Soluciones  en Ventas y Distribución <br> ​
							* Soluciones en Compras e Inventarios ​ <br>
							* Soluciones para la planeación de la producción​ <br>
							* Soluciones para el control de Calidad​ <br>
							* Soluciones para el control del Mantenimiento​ <br>
							* Portales para Clientes CRM​ <br>
							* Portales para proveedores SRM​ <br>
							* Analíticos para la toma de decisión Directiva​ <br>
							* Consultoría por tiempo y materiales en cualquier especialidad​ <br>
							* Upgrades​ <br>
							* Migraciones​ <br>
							* Soportes​ <br>
							* Roll-Outs​ <br>
							* Implementaciones​ <br>
							* Capacitación​ <br>
							* Hosting <br>
				  		</p>
				  	</li>

				</ol>

		  	</div>
		</div>
		<div>
			<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>CONSULTORÍA E IMPLEMENTACIÓN SAP</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
			<img class="icon-float" src="images/icons/document.png">
			<span style="background: url('images/img/FOTO 13.jpg');"></span>
		  	<h2>CONSULTORÍA E IMPLEMENTACIÓN SAP</h2>
		  	<br>

		  	<div>
				<ol class="square-sap-02">	
			
				  	<li>
				  		<h3>CONSULTORÍA EN SOLUCIONES SAP</h3>
				  		<p>
				  			* Soluciones en la cadena de suministro, logística ​ <br>
							* Soluciones Financieras y de costos ​ <br>
							* Soluciones  en Ventas y Distribución <br> ​
							* Soluciones en Compras e Inventarios ​ <br>
							* Soluciones para la planeación de la producción​ <br>
							* Soluciones para el control de Calidad​ <br>
							* Soluciones para el control del Mantenimiento​ <br>
							* Portales para Clientes CRM​ <br>
							* Portales para proveedores SRM​ <br>
							* Analíticos para la toma de decisión Directiva​ <br>
							* Consultoría por tiempo y materiales en cualquier especialidad​ <br>
							* Upgrades​ <br>
							* Migraciones​ <br>
							* Soportes​ <br>
							* Roll-Outs​ <br>
							* Implementaciones​ <br>
							* Capacitación​ <br>
							* Hosting <br>
				  		</p>
				  	</li>

				</ol>

		  	</div>
		</div>

	  	<!-- CONTENIDO SOLUCIONES HOSPITALARIAS​ -->
	  	<a id="hospitales"></a>
	  	<div>
	  		<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>SOLUCIONES HOSPITALARIAS</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
	  		<img class="icon-float" src="images/icons/informe-medico.png">
			<span style="background: url('images/img/FOTO 14.jpg');"></span>
		  	<h2>SOLUCIONES HOSPITALARIAS</h2>
		  	<p>
		  		Ofrecemos soluciones de tecnología diseñadas para la salud, que ayuda a mejorar los procesos administrativos, médicos y de atención a los pacientes. Nuestras soluciones hospitalarias utilizan tecnologías inteligentes para ayudar a nuestros clientes a mejorar sus procesos y calidad de servicios sin importar el tamaño de su negocio.
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3>CATALOGO DE SOLUCIONES</h3>
				  		<p style="text-align: left;">
				  			* Sistemas de Información Hospitalaria<br>
				  			* Sistemas de Tele Medicina<br>
				  			* ​Sistemas de Tele Enseñanza<br>
				  			* Sistema Paciente Enfermera<br>
				  		</p>
				  	</li>

				  	<li>
				  		<img src="images/icons/dispositivo.png">
				  		<img src="images/icons/informe-medico.png">
				  	</li>

				  	<li>
				  		<img src="images/icons/medical-assistance.png">
				  		<img src="images/icons/healthcare.png">
				  	</li>

				</ol>

		  	</div>
		</div>

		<div>
			<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>SOLUCIONES HOSPITALARIAS</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
			<img class="icon-float" src="images/icons/informe-medico.png">
			<span style="background: url('images/img/FOTO 15.jpg');"></span>
		  	<h2><img style="width: 4vw;" src="images/icons/HIS.png"> TUTUM HIS</h2>
		  	<p>
		  		Es una solución diseñada para administrar y controlar todas y cada una de las áreas que componen un Hospital o Clínica; esta solución permite la seguridad hacia el paciente y la precisión diagnóstica, optimizando los procesos asistenciales, colaborando en la reducción de costos de la atención en salud. 
		  	</p>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3>Módulos administrativos</h3>
				  		<p>
				  			* Adm. y proveedores​ <br>
							* Almacén ​ <br>
							* Cuentas por pagar​ <br>
							* Cuentas por cobrar​ <br>
							* Compras​ <br>
							* Caja y facturación​ <br>
							* Costos​ <br>
							* Ccontabilidad <br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Módulos clínicos​</h3>
				  		<p>
							* Admisión hospitalaria​<br>
							* Urgencias​<br>
							* Laboratorio​<br>
							* Banco de sangre​<br>
							* Imagenología​<br>
							* Consulta externa​<br>
							* Trabajo social​<br>
							* Administración de quirófanos​<br>
							* Enfermería​<br>
							* Indicaciones médicas ​<br>
							* Dietas y cocina​ <br>
							* Expediente clínico electrónico<br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Módulos de configuración</h3>
				  		<p>
							* Seguridad​<br>
							* Importador de datos​<br>
							* Administración del sistema​<br>
							* Flujo de trabajo​<br>
							* Cuestionarios​<br>
							* Formas​<br>
							* Interoperabilidad<br>
				  		</p>
				  	</li>

				</ol>

		  	</div>
		</div>

	  	<!-- CONTENIDO CIBERSEGURIDAD -->
	  	<a id="ciberseguridad"></a>
	  	<div>
	  		<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>CIBER SEGURIDAD</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
			<span style="background: url('images/img/FOTO 10.jpg');"></span>
		  	<h2>CIBERSEGURIDAD</h2>
		  	<p>
		  		Ofrecemos soluciones de tecnología diseñadas para la 
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3>DIFINIR INFORMACION</h3>
				  		<p>
				  			* <br>
				  			* <br>
				  			* ​<br>
				  			* <br>
				  		</p>
				  	</li>

				  	<li>
				  		<h3>DIFINIR INFORMACION</h3>
				  		<p>
				  			
				  		</p>
				  	</li>

				  	<li>
				  		<h3>DIFINIR INFORMACION</h3>
				  		
				  	</li>

				</ol>

		  	</div>
		</div>

	  	<!-- CONTENIDO FABRICA DE SOFTWARE -->
	  	<a id="fabrica"></a>
	  	<div>
	  		<!-- DESCARGA DE PRESENTACION -->
		  	<div class="download-pdf-contenido">
			    <i class="fas fa-info-circle"></i>
			    <ol>
			      <li>
			        <p>FABRICA DE SOFTWARE</p>
			        <img src="images/icons/download.png">
			      </li>
			    </ol>
			</div>
			<span style="background: url('images/img/FOTO 05.jpg');"></span>
		  	<h2>FABRICA DE SOFTWARE</h2>
		  	<p>
		  		Te ayudamos a desarrollar e implementar tecnología para optimizar tus operaciones y maximizar la rentabilidad, integrando tus actuales servicios, para que tú te dediques a tu negocio.<br><br>Contamos con desarrolladores expertos, y con vasta experiencia en industrias específicas, con desarrollos apegados a las mejores prácticas de seguridad, desarrollo ágil entre otras. 
		  	</p>

		  	<br>

		  	<div>
		  		
				<ol>	
				  	<li>
				  		<h3>Desarrollo Web Personalizado</h3>
				  		<p>
				  			Nuestras soluciones de desarrollo web facilitan los flujos de trabajo, aumentan los ingresos y optimizan las operaciones comerciales, lo anterior bajo desarrollos que cumplen con los principales estándares de seguridad aplicables a cada sector de la industria.
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Mantenimiento de Aplicaciones</h3>
				  		<p>
				  			Nuestros servicios de mantenimiento y modernización están diseñados para garantizar la escalabilidad, el rendimiento y la sostenibilidad de toda su infraestructura de software, manteniendo así la vigencia de los distintos aplicativos al interior de la organización, bajo un esquema de mejores prácticas de CMMI.
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Aplicaciones Personalizadas</h3>
				  		<p>
				  			Con base en la experiencia de nuestro equipo de más de 20 años en el desarrollo de aplicaciones y , bajo un esquema de mejores prácticas del modelo CMMI, podemos ofrecer aplicaciones a la medida tanto web como de escritorio, flexibles e interoperables, conectando de ser necesarios desarrollos in house preexistentes.
				  		</p>
				  	</li>

				  	<li>
				  		<h3>Implementación y Despliegue</h3>
				  		<p>
				  			Diseñamos un plan de implementación en conjunto con nuestra Mesa de Servicios a profundidad, evaluando las necesidades de su empresa, para así brindar tecnologías mejoradas a los usuarios finales. En nuestras implementaciones consideramos los esquemas de regreso en caso de existir algún impacto en servicios productivos, apegándonos al ciclo de vida del servicio marcado en el marco de referencia ITIL V4.
				  		</p>
				  	</li>

				</ol>

		  	</div>

		  	<div style="display: none;">
		  		
				<ol>	

				  	<li>
				  		<h3>Implementación y Despliegue</h3>
				  		<p>
				  			Diseñamos un plan de implementación en conjunto con nuestra Mesa de Servicios a profundidad, evaluando las necesidades de su empresa, para así brindar tecnologías mejoradas a los usuarios finales. En nuestras implementaciones consideramos los esquemas de regreso en caso de existir algún impacto en servicios productivos, apegándonos al ciclo de vida del servicio marcado en el marco de referencia ITIL V4.
				  		</p>
				  	</li>

				</ol>

		  	</div>
	  	</div>
	</div>
</div>